...
def parse_option():
    ...
    parser.add_argument(
        ...
        type=str,
        default='CWRU_signal',
        choices=['cifar10', 'cifar100', 'gear_fault_signal', 'path', 'CWRU_signal', 'XMU_Motor_signal'],
        help='dataset')
    ...
    opt.model_path = './save/' + opt.data_folder.split('/')[-3] + '/SigSiam/{}_models'.format(opt.dataset)
    opt.tb_path = './save/' + opt.data_folder.split('/')[-3] + '/SigSiam/{}_tensorboard'.format(opt.dataset)
    ...
    opt.model_name = 'drsn_{}_{}_lr_{}_decay_{}_bsz_{}_temp_{}_trial_{}_{}'.\
        format(opt.method, opt.dataset, opt.learning_rate,
               opt.weight_decay, opt.batch_size, opt.temp, opt.trial, time.strftime('%m-%d-%H-%M',time.localtime(time.time())))
...
def set_loader(opt):
    ...
    if opt.dataset == 'XMU_Motor_signal':
        train_dataset = MotorSignalDataset(opt.data_folder)
    else:
...
def train(train_loader, model, criterion, optimizer, epoch, opt):
    ...
    for idx, (signals, labels) in enumerate(train_loader):
        ...
        bsz = labels.shape[0]
        # signals = torch.cat([signals[0], signals[1]], dim=0)
        ...
        # features = model(signals)[0]
        # f1, f2 = torch.split(features, [bsz, bsz], dim=0)
        # features = torch.cat([f1.unsqueeze(1), f2.unsqueeze(1)], dim=1)
        # loss = criterion(features, epoch=epoch)
