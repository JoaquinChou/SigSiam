...
from utils.util import set_optimizer, set_optimizer_fine_tuning
from networks.classifiers import LinearClassifier
...
def parse_option():
    ...
    # model dataset
    parser.add_argument('--model', type=str, default='sigsiam_DRSN_CW')
    ...
    parser.add_argument('--dataset', type=str, default='CWRU_signal',
                        choices=['cifar10', 'cifar100', 'gear_fault_signal', 'CWRU_signal', 'XMU_Motor_signal'], help='dataset')
    ...
    opt.model_name = 'drsn_{}_{}_lr_{}_decay_{}_bsz_{}'.\
    ...
    if opt.dataset == 'cifar10':
        ...
        opt.n_cls = 100
    elif opt.dataset == 'gear_fault_signal':
        opt.n_cls = 9
    elif opt.dataset == 'CWRU_signal':
        opt.n_cls = 10
    elif opt.dataset == 'XMU_Motor_signal':
        opt.n_cls = 6
    else:
...
def train(train_loader, model, classifier, criterion, optimizer, optimizer_backbone, epoch, opt):
    """one epoch training"""
    if opt.fine_tuning:
        model.train()
...
def validate(val_loader, model, classifier, criterion, opt):
    ...
    with torch.no_grad():
        ...
        for idx, (signals, labels) in enumerate(val_loader):
            ...
            # forward
            output = classifier(model(signals)[2])
            loss = criterion(output, labels)
...
def main():
    ...
    # build optimizer
    if opt.fine_tuning:
        optimizer_backbone = set_optimizer_fine_tuning(model)
    ...
    for epoch in range(1, opt.epochs + 1):
        ...
        loss, acc = train(train_loader, model, classifier, criterion,
                          optimizer, optimizer_backbone, epoch, opt)
        time2 = time.time()
